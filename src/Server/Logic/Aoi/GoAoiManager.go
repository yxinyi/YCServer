...
func NewGoAoiManager(width_, height_, block_size_ float64) *GoAoiManager {
	_mgr := &GoAoiManager{
		...
		m_action_list:   queue.NewSyncQueue(),
	}
...
func (mgr *GoAoiManager) Enter(enter_ GoAoiObj, pos_ YMsg.PositionXY) {
...
func (mgr *GoAoiManager) Quit(quit_ GoAoiObj, pos_ YMsg.PositionXY) {
	_current_index := mgr.CalcIndex(pos_)
	_cell := mgr.m_aoi_list[_current_index]
...
func (mgr *GoAoiManager) Move(move_ GoAoiObj, pos_ YMsg.PositionXY) {
...
func (mgr *GoAoiManager) CalcIndex(xy_ YMsg.PositionXY) uint32 {

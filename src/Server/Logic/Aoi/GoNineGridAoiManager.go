...
func NewGoNineGirdAoiCellManager(width_, height_, block_size_ float64) *GoNineGirdAoiManager {
	_mgr := &GoNineGirdAoiManager{
		...
		M_action_out_:   queue.NewSyncQueue(),
...
func (mgr *GoNineGirdAoiManager) updateCell(enter_ *GoAoiObj, cell_set_ map[uint32]struct{}) {
	{
		...
		for _it := range cell_set_ {
			_enter_map_sync = tool.Uint64MapUint64SetMerge(_enter_map_sync, mgr.getObjInViewRangeMap(enter_, _it))
		}
	...
	}
	{
		...
		for _it := range cell_set_ {
			_quit_map_sycn = tool.Uint64MapUint64SetMerge(_quit_map_sycn, mgr.getObjNotInViewRangeMap(enter_, _it))
		}

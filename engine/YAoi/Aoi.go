...
/*func (cell *AoiCell) watchTower(enter_ uint32) {
	cell.m_obj_list[enter_] = make(map[uint32]struct{})
	for _it := range cell.m_obj_list {
		if cell.M_add_watch_callback(enter_, _it) {
			cell.m_obj_list[enter_][_it] = struct{}{}
			cell.M_enter_callback(enter_, _it)
		}
		if cell.M_add_watch_callback(_it, enter_) {
			cell.m_obj_list[_it][enter_] = struct{}{}
			cell.M_enter_callback(_it, enter_)
		}
	}
}

func (cell *AoiCell) quitTower(enter_ uint32) {
	_watch_list := cell.m_obj_list[enter_]
	for _it := range _watch_list {
		if enter_== _it {
			delete(cell.m_obj_list[_it], enter_)
			continue
		}
		cell.M_quit_callback(enter_, _it)
		_, exists := cell.m_obj_list[_it][enter_]
		if exists {
			cell.M_quit_callback(_it, enter_)
		}
		delete(cell.m_obj_list[_it], enter_)
	}
	delete(cell.m_obj_list, enter_)
}

func (cell *AoiCell) updateCell(enter_ uint32) {
	for _it := range cell.m_obj_list {
		if cell.M_add_watch_callback(enter_, _it) {
			_, exists := cell.m_obj_list[enter_][_it]
			if exists {
				cell.M_move_callback(enter_, _it)
			} else {
				cell.m_obj_list[enter_][_it] = struct{}{}
			...
			if exists {
				cell.M_quit_callback(enter_, _it)
				delete(cell.m_obj_list[enter_], _it)
			}
		}
